...
public class DefaultTrackableEventHandler : MonoBehaviour, ITrackableEventHandler
{
    ...
    protected virtual void Update()
    {
        ...
        if (distance < 0.073)
        {       //If distance is below 0.073, that is lower than the camera can track. This is mostly for initialization since distance defaults to a really low number.
        ...
        if (distance > 0.5)
        {       //If distance is above maximum threshold (at the moment it is 0.5)
        ...
        {   //If distance is below minimum threshold, OnTrackableStateChanged() is called so new image target can be tracked and added sugar displayed on screen
        ...
        TrackableBehaviour.Status newStatus)
    {
            ...
            distance < 0.3)
        {
            ...
            if (count == 3)
            {                       //If count makes it to 3, then most likely distance has been exactly the same for 4 frames in a row (0,1,2,3)
                if (prevDist[0] == prevDist[3])
                {       //If the first set prevDist is equal to newest set prevDist, then distance has definitely been exactly the same for 4 frames in a row
    ...
    protected virtual void OnTrackingFound()	//Starts tracking and displays added sugar values
    {
        ...
        //RSI = gameObject.AddComponent<RetrieveIngredients>();
        //Text text_mesh_label = GameObject.Find ("Text").GetComponent<Text> ();
        //text_mesh_label.text = RSI.SugarIngredients (imgTargetName);
    ...
    {       //Stops tracking and displaying added sugar values without messing with OnTrackableStateChanged method (Called outside of the OnTrackableStateChanged method)
        var rendererComponents = GetComponentsInChildren<Renderer>(true);
        var colliderComponents = GetComponentsInChildren<Collider>(true);
        var canvasComponents = GetComponentsInChildren<Canvas>(true);
